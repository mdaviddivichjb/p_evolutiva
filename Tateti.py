...
#  * Est_i (1..c_estados) * Entrada (1..27) * Salida (1..9) * Est_f (1..c_estados) *
...
def genero_tabla_est (cant_ent):
    tabla_est = []
    for aux1 in range(1,(cant_ent +1)):
        for aux2 in range(0,27):
...
def resultado_algoritmo(tablero,tabla_estados,l_vacios):
...
def genero_poblacion(c_indiv,c_estados):
    individuos = []
...
def juego_ta_te_ti(tabla_mejor,tablero):
   ...
   while (partido_sigue and not(hay_ganador)):
       ...
       hay_ganador = busco_ganador(tablero_res)

       if (partido_sigue and not(hay_ganador)):
           res = resultado_algoritmo(tablero,tabla_mejor,l_vacios)
           jugada_maq = res[0]
           estado = res[1]
       ...
       partido_sigue = hay_vacios (tablero_res1)
       hay_ganador = busco_ganador(tablero_res1)
